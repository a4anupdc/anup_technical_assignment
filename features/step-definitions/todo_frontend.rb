Given('I visit todo applications home page') do
  ...
  if(page.has_css?('.footer'))
	  @count_todo_items = find('.todo-count strong').text.to_i
  end
end
...
Then('I should see {string} item added to todo list') do |text|
  expect(page).to have_css("ul.todo-list li:last-child label", text: "#{text}")
...
Given('I make sure that {string} logo is present on screen') do |logo_text|
  expect(page).to have_css('h1', text: "#{logo_text}")
end
...
When('I double click on todo item {string} and update its value to {string}') do |old_text, new_text|
  @text = new_text
  find(".todo-list li:last-child label", text: "#{old_text}").double_click
...
Then('I should see updated item with text {string} in todo list') do |new_text|
  expect(page).to have_css(".todo-list label", text: "#{new_text}")
end

When('I delete last item added to todo list') do
  find('.todo-list li:last-child label', text: "#{@text}").hover
  find('button.destroy').click
end

Then('I should not see deleted todo item on screen') do
  expect(page).to have_no_css('.todo-list li:last-child label', text: "#{@text}")
end
...
Then('I should see to-do counter {string} by {int}') do |operation, number|
	...
	expect(page).to have_css('.todo-count strong', text: "#{@count_todo_items}")
end
...
Then('I should see that all todo tasks are marked as completed') do
  ...
  expect(page).to has_button?('Clear completed')
end
...
When('I click on {string} link') do |link_text|
  link_text.eql?('Clear completed') ? (click_button "#{link_text}") : (click_link "#{link_text}")
end
